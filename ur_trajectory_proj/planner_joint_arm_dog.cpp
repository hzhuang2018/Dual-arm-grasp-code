#include  "planner_joint_arm_dog.h"
#include "collision_dualarm.h"
#include <opencv2/core/eigen.hpp>
//#include "C:\3rdparties\opencv\include\opencv2\core\eigen.hpp"
	void planner_joint_arm_dog::setStart(double x, double y, double z, double Rx, double Ry, double Rz)
	{
	...
	void planner_joint_arm_dog::setStart(Eigen::VectorXd rob_q, Eigen::VectorXd rob2_q)
	{
	...
	void planner_joint_arm_dog::setGoal(double x, double y, double z, double Rx, double Ry, double Rz,
...
//cout << Rccc1 << endl;
	...
	void planner_joint_arm_dog::updateMap(std::shared_ptr<fcl::CollisionGeometry<double>> map)
	{
	...
	planner_joint_arm_dog::planner_joint_arm_dog()
	{
		...
		//newSpace->addSubspace(ompl::base::StateSpacePtr(new ompl::base::RealVectorStateSpace(6)), 1.0);
		//newSpace->addSubspace(ompl::base::StateSpacePtr(new ompl::base::RealVectorStateSpace(6)),1.0);
		...
		ompl::base::StateSpacePtr robv6(new ob::RealVectorStateSpace(6));//RealVectorStateSpace
		// construct a compound state space using the overloaded operator+
		ompl::base::StateSpacePtr space = robv6 + se3;



		//解的状态空间
		//space = ob::StateSpacePtr(new ob::RealVectorStateSpace(12));//RealVectorStateSpace
		//space2 = ob::StateSpacePtr(new ob::RealVectorStateSpace(6));//RealVectorStateSpace
		//space = space1 + space2;
		...
		// 搜索的三维范围设置
		ob::RealVectorBounds bounds(9);

		//bounds for robot 1
		bounds.setLow(0, -M_PI/2);
		bounds.setHigh(0, M_PI/2);
		bounds.setLow(1, -5*M_PI/6);
		bounds.setHigh(1, -M_PI / 4);
		...
		//bounds for robot 2
		bounds.setLow(0 + 6, - M_PI/2);
		bounds.setHigh(0 + 6,  M_PI/2);
		bounds.setLow(1 + 6,  -3* M_PI / 4);
		bounds.setHigh(1 + 6, 0 );
		bounds.setLow(2 + 6, 0);
		bounds.setHigh(2 + 6, M_PI);
		//bounds.setLow(3 + 6, - M_PI);
		//bounds.setHigh(3 + 6,  M_PI);
		//bounds.setLow(4 + 6, - M_PI);
		//bounds.setHigh(4 + 6,  M_PI);
		//bounds.setLow(5 + 6, - M_PI);
		//bounds.setHigh(5 + 6,  M_PI);
		...
		//space->allocSubspaceStateSampler()->setBounds();
		
		space->as<ob::RealVectorStateSpace>()->setBounds(bounds);
		//space2->as<ob::RealVectorStateSpace>()->setBounds(bounds);
		...
		si->setStateValidityChecker(std::bind(&planner_joint_arm_dog::isStateValid, this, std::placeholders::_1));
		si->setStateValidityCheckingResolution(0.008);//0.001(最近用的）  0.02
		si->setup();
		...
		pdef->setOptimizationObjective(planner_joint_arm_dog::getThresholdPathLengthObj(si));//getThresholdPathLengthObj getPathLengthObjWithCostToGo
	...
	void planner_joint_arm_dog::replan(deque<double>& _path_point)
	{
	...
	void planner_joint_arm_dog::plan(deque<double> & _path_point)
	{
		...
		if (solved)
		{
			...
			pathBSpline->smoothBSpline(*path_smooth, 5);//以前一直是3
	...
	bool planner_joint_arm_dog::isStateValid(const ob::State* state)
	{ 
	...
	// paths.
	ob::OptimizationObjectivePtr planner_joint_arm_dog::getThresholdPathLengthObj(const ob::SpaceInformationPtr& si)
	{
	...
	ob::OptimizationObjectivePtr planner_joint_arm_dog::getPathLengthObjWithCostToGo(const ob::SpaceInformationPtr& si)
	{
		ob::OptimizationObjectivePtr obj(new ob::PathLengthOptimizationObjective(si));
