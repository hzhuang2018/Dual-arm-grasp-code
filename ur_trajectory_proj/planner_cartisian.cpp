...
#include "collision.h"
	void planner_cartisian::setStart(double x, double y, double z)
	{
	...
	void planner_cartisian::setGoal(double x, double y, double z)
	{
	...
	void planner_cartisian::updateMap(std::shared_ptr<fcl::CollisionGeometry<double>> map)
	{
		tree_obj = map;
	}
	// Constructor
	planner_cartisian::planner_cartisian(CartesianSpaceTrackUR* ur)
	{
		_ur = ur;
		is_cartisian_planner = true;
		//四旋翼的障碍物几何形状
		...
		si->setStateValidityChecker(std::bind(&planner_cartisian::isStateValid, this, std::placeholders::_1));
		...
		pdef->setOptimizationObjective(planner_cartisian::getThresholdPathLengthObj(si));//getThresholdPathLengthObj getPathLengthObjWithCostToGo
	
		std::cout << "Initialized: " << std::endl;
	}
	// Destructor
	planner_cartisian::~planner_cartisian()
	{
	}
	void planner_cartisian::replan(void)
	{
	...
	void planner_cartisian::plan(deque<double> & _path_point)
	{

		// create a planner for the defined space
		og::RRTstar* rrt = new og::RRTstar(si);//InformedRRTstar  LBKPIECE1  PRMstar
	...
	bool planner_cartisian::isStateValid_backup(const ob::State* state)
	{
		// cast the abstract state type to the type we expect
	...
	bool planner_cartisian::isStateValid(const ob::State* state)
	{ 
		// cast the abstract state type to the type we expect
		const ob::SE3StateSpace::StateType* se3state = state->as<ob::SE3StateSpace::StateType>();
	...
	// paths.
	ob::OptimizationObjectivePtr planner_cartisian::getThresholdPathLengthObj(const ob::SpaceInformationPtr& si)
	{
	...
	ob::OptimizationObjectivePtr planner_cartisian::getPathLengthObjWithCostToGo(const ob::SpaceInformationPtr& si)
	{
		ob::OptimizationObjectivePtr obj(new ob::PathLengthOptimizationObjective(si));
