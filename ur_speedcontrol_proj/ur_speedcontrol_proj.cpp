...
{
	double qa[6] = { 0,-M_PI / 2,0,-M_PI / 2, 0, 0 }, qb[6] = { M_PI / 3,-M_PI / 3,M_PI / 4,-M_PI / 3, 0, 0 }, qc[6], qd[6];//q为关节角度
...
#define N  40 //精度为小数点后面2位
	...
	while (1)
	{
		...
		while ((icounter < itotaltime + 1))
		{
			...
			{
				//笛卡尔空间直线运行过程中，发现机械臂跟不上下发的位置，根据观察能到0.08

				//if (abs(q_pos_sequence[im + icounter * 6]- df_q_act_post[im])>0.35)//0.025对应最大速度180度的弧度。
				//	controller_ur->q_pos_target[im] = df_q_act_post[im]+ 0.35*SIGN(q_pos_sequence[im + icounter * 6] - df_q_act_post[im]);
				//else
			...
			for (int i = 0; i < 6; i++)
			{
				controller_ur->qcd_pre[i] = controller_ur->qcd[i];
			}
		}
		for (int i = 0; i < 6; i++)
		{
			controller_ur->qcd[i] = 0;
		}
	...
	delete[] q_speed_sequence;
	delete[] q_acc_sequence;
	boost::posix_time::ptime stop_time = boost::get_system_time();
	controller_ur->stop(stop_time);
}
