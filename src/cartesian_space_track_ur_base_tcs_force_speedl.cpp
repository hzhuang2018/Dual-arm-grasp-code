...
namespace Robot {
	...
	bool CartesianSpaceTrackUR::ik_with_q(Eigen::Matrix<double, 4, 4>& Matrix_end_point,double * desire_q)
	{
		...
		for (int mm = 0; mm < 6; mm++)
		{
			...
			std::cout << "Currend joiint postion:= " << q_former_choosed_pos[mm] << std::endl;
		}
		num_sols = inverse(T_Temp, q_sols, 0.0);
		//std::cout << "num_sol=" << num_sols << std::endl;
		//for (int i = 0; i < 48; i++)
	...
	void CartesianSpaceTrackUR::kinematic_controller_hw()
	{
		...
		x_smooth_force[counter] = tool_coordinate_system_force_[0];// tool_force_[0];
		y_smooth_force[counter] = tool_coordinate_system_force_[1];//tool_force_[1];
		z_smooth_force[counter] = tool_coordinate_system_force_[2];//tool_force_[2];
		...
		case CONTROLLER_TYPE::FULL_POSE:
		{
			...
			{ 
				dx << Kp *(x(0) - x_des(0))- Spring_k  * base_force_torque_error(0) +0.5* Spring_k *(base_force_torque_error(0) - x_force_error_dot), x(1) - x_des(1)- Spring_k*0 * y_force_error, x(2) - x_des(2)- Spring_k* 0 * z_force_error, d_quat.x(), d_quat.y(), d_quat.z();// 原始
				xx_desd << xd_des(0), xd_des(1), xd_des(2), w_des(0), w_des(1), w_des(2);//期望位置速度和期望姿态速度
				...
				dx(4) = -0.1 * tool_force_[4] * (not (bool)Kp) + Kp * 0.5 * 2 * (-qua_R_cur.y() * (qua_R_cur.w() - qua_R_des.w()) + qua_R_cur.z() * (qua_R_cur.x() - qua_R_des.x()) + qua_R_cur.w() * (qua_R_cur.y() - qua_R_des.y()) - qua_R_cur.x() * (qua_R_cur.z() - qua_R_des.z()));
				dx(5) = -0.1 * tool_force_[5] * (not (bool)Kp) + Kp * 0.5 * 2 * (-qua_R_cur.z() * (qua_R_cur.w() - qua_R_des.w()) - qua_R_cur.y() * (qua_R_cur.x() - qua_R_des.x()) + qua_R_cur.x() * (qua_R_cur.y() - qua_R_des.y()) + qua_R_cur.w() * (qua_R_cur.z() - qua_R_des.z()));		
			}
			else
			{
		...
		x_force_error_dot = base_force_torque_error(0);
	...
	void CartesianSpaceTrackUR::kinematic_jointPcontroller()
	{
		...
		case CONTROLLER_TYPE::FULL_POSE:
		{
			...
			//q:从机器人读关节位置，正解得到的实际位置，Q_des由外面传进来的期望位置
			...
			q_deviation << jKp[0] * (Q_des[0] - q_current[0]) + jKd[0] * (Q_des[0] - q_current[0] - jLastErr[0]),
						   jKp[1] * (Q_des[1] - q_current[1]) + jKd[1] * (Q_des[1] - q_current[1] - jLastErr[1]),
						   jKp[2] * (Q_des[2] - q_current[2]) + jKd[2] * (Q_des[2] - q_current[2] - jLastErr[2]),
				           jKp[3] * (Q_des[3] - q_current[3]) + jKd[3] * (Q_des[3] - q_current[3] - jLastErr[3]),
				           jKp[4] * (Q_des[4] - q_current[4]) + jKd[4] * (Q_des[4] - q_current[4] - jLastErr[4]),
				           jKp[5] * (Q_des[5] - q_current[5]) + jKd[5] * (Q_des[5] - q_current[5] - jLastErr[5]);
