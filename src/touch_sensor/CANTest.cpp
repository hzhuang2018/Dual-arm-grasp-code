...
int devIndex=0; 
int canIndex=CAN_CHANNEL_START;
...
eGinkgoCanDev vt_usb_can_check_dev(PVCI_BOARD_INFO_EX pInfo){
	eGinkgoCanDev ret;
...
int SetBaudRate(int baudrate, VCI_INIT_CONFIG_EX *config_ex, PVCI_BOARD_INFO_EX pInfo)
{
	int i, ret=0; 
	...
	ginkgoDev = vt_usb_can_check_dev(pInfo);
	SPRINTF(("ginkgoDev: %d\n", ginkgoDev));
...
void WINAPI GetDataCallback(uint32_t DevIndex,uint32_t CANIndex,uint32_t Len)
{
	...
	canIndex = 0;
	int ReadDataNum;
	int DataNum = VCI_GetReceiveNum(devType, devIndex, canIndex);
	VCI_CAN_OBJ* pCAN_ReceiveData = (VCI_CAN_OBJ*)malloc(DataNum * sizeof(VCI_CAN_OBJ));
	if ((DataNum > 0) && (pCAN_ReceiveData != NULL)) {
		ReadDataNum = VCI_Receive(devType, devIndex, canIndex, pCAN_ReceiveData, DataNum);
...
int touch_sensor_read_data()
{
	...
	//Open device
    Status = VCI_OpenDevice(devType, devIndex, 0);
    ...
    SetBaudRate(1000, &CAN_InitEx, &CAN_BoardInfo); 
	...
	#if CAN_CALLBACK_READ_DATA
		VCI_RegisterReceiveCallback(devIndex, GetDataCallback);
...
//   Status = VCI_Transmit(VCI_USBCAN2,0,0,CAN_SendData,0);
	...
	sleep(0.01);
            ...
            Status = VCI_ReadCANStatus(devType, devIndex, canIndex, &CAN_Status);
            if (Status == STATUS_ERR)
            {
